...
import com.agriweb.agripriceshop.domain.Category;
...
public class ItemApiController {
    ...
    // 아이템 등록 API
    @Operation(summary = "아이템 등록 메서드", description = "아이템 등록 메서드입니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "successful operation"),
            @ApiResponse(responseCode = "400", description = "bad request operation")
    })
    @PostMapping("/api/items/")
    public ResponseEntity<String> create(@RequestBody ItemDto dto, String loginId) {
        Member member = memberService.findOnebyLoginId(dto.getLoginId());
        Category category =
        Item item = Item.createItem(dto, member, category);
